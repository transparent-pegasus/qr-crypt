...
export const LOCALES = {
  ja: {
    ...
    strings: {
      ...
      "install.a.body":
        "これからオフラインにする端末のブラウザーでこのサイトを開き、「インストール」または「ホーム画面に追加」を選びます。終わったら、その端末をネットワークから完全に切り離し、二度と接続しないでください。一度オンラインにした端末の機内モードは信用するべきではありません。",
      ...
      "install.b.body":
        "信頼できるパソコンで署名付きのリリースをダウンロードし、その場で署名とチェックサムを確認します。確認に使う値は、ダウンロードとは別の経路で入手してください。オフライン端末へ移して解凍し、そのフォルダーを127.0.0.1だけに向けた静的Webサーバーで配信します。同梱のINSTALL.txtが求めるとおり、独立した経路で用意した信頼できるサーバーを使い、同梱のヘッダーとリダイレクトの設定を適用してください。index.htmlをファイルとして直接開く方法は使えません。その場合ブラウザーの保存領域の分離が働かず、別のローカルHTMLファイルから鍵を読み出せることがあります。運ぶのに使うUSBメモリーやSDカードも、信頼できるものだけを使ってください。そこに細工できる相手は、アプリも改ざんできます。",
      "install.b.link": "最新リリースを取得",

      "flow.eyebrow": "使い方",
      "flow.heading": "文章が相手に届くまで",
      "flow.lede":
        "この6手順のあいだ、読める状態の文章がオフライン端末から出ることはありません。境目を越えるのは暗号文と、その運搬に必要な枠組みの情報だけです。方式で変わるのは鍵の受け渡しで、AES-256は共通鍵を一方向に読み取れば済みます。ポスト量子IDは、署名を付ける場合と双方向にやり取りする場合に、互いの公開鍵を読み取る必要があります。以下はML-KEM-1024の場合です。",
      "flow.online": "オンライン",
