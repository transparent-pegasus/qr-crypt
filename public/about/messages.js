...
export const LOCALES = {
  ja: {
    ...
    strings: {
      ...
      "install.a.body":
        "これは既定の導入方式です。信頼できるパソコンで署名付きのリリースをダウンロードし、その場で署名とチェックサムを確認します。確認に使う値は、ダウンロードとは別の経路で入手してください。オフライン端末へ移して解凍し、そのフォルダーを127.0.0.1だけに向けた静的Webサーバーで配信します。同梱のINSTALL.txtが求めるとおり、独立した経路で用意した信頼できるサーバーを使い、同梱のヘッダーとリダイレクトの設定を適用してください。index.htmlをファイルとして直接開く方法は使えません。その場合ブラウザーの保存領域の分離が働かず、別のローカルHTMLファイルから鍵を読み出せることがあります。運ぶのに使うUSBメモリーやSDカードも、信頼できるものだけを使ってください。そこに細工できる相手は、アプリも改ざんできます。",
