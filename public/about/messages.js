...
export const LOCALES = {
  ja: {
    ...
    strings: {
      ...
      "seam.label": "ネットワークが途切れる場所",
      "seam.body1": "この境界を越えるのは光だけ —",
      "seam.body2": "一方の画面に表示されたQRコード、もう一方のカメラ。",
      ...
      "locks.pq.body":
        "量子計算機の大規模化と解読技術の進歩により、現在広く使われている公開鍵暗号による鍵共有は、いずれ破られると私たちは考えています。ML-KEM-1024は、その将来に備えて設計された方式です。国家レベルの解読能力を持つ相手に対しても、メッセージが長期間読めないままであることが期待されます。そのぶん処理とデータ量は大きく、QRコードは複数枚に分かれます。",
      ...
      "install.b.body":
        "これからオフラインにする端末でこのサイトを開き、「インストール」または「ホーム画面に追加」を選んでから、すべてのネットワークから切り離します。この方法には、受け取った側が実行できる完全性検証がありません。配信元、TLS、CDNのいずれかを掌握した攻撃者は、標的にした1台だけへ検出されずに改変バンドルを配信でき、Service Workerが改変物を永続化します。端末には、アプリの取得元である実在のサーバーがひも付いたまま残ります。再接続すると同一オリジンへ到達性プローブが送られ、wipeはsentinelの応答を待ってから発火するため、ビーコンは必ずwipeより先に出ます。方法Aのオリジンは127.0.0.1だけです。専用サーバーを停止し、予約したポートを再利用しなければ、そこに応答する相手はいません。機内モードだけで端末が信頼できるようになるとは考えないでください。",
      ...
      "warn.two.body":
        "保存している鍵とデータの削除を試み、いずれかのステップが失敗した場合はその旨を通知します。ただし、削除されたデータが完全に消えるとは限りません。痕跡はハードウェアから復元できることがよくあります。本当に復元不可能にする必要がある場合は、メディアに適した方法（NIST SP 800-88）でストレージをワイプするか、デバイスを物理的に破壊してください。",
