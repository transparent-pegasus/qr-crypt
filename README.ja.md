...
オフライン暗号化 QR PWA。端末上で平文を暗号化し、暗号文・鍵素材を QR として表示・読取する Progressive Web App です。アプリ管理下の IndexedDB/localStorage にはメッセージ暗号文を永続化しません。利用者が明示した PNG/ZIP ダウンロードとクリップボードは OS・ブラウザー・同期先に残り得て、wipe/purge の対象外です。
...
| スイート | 内容 | 平文上限（UTF-8 bytes） | 備考 |
| --- | --- | ---: | --- |
| AES-256-GCM | 対称暗号のみ | EC-L: 2,010; EC-M: 1,543; EC-Q: 1,042; EC-H: 750 | **既定**（`A256GCM`）。選択した QR 誤り訂正レベルで単一 OCM1 の上限が決まる |
| ML-KEM-1024 + HKDF-SHA256 + AES-256-GCM | ポスト量子 KEM ハイブリッド | 120,000 | 選択可（`MLKEM1024_A256GCM`） |
| 上記 + ML-DSA-87 署名 | sign-then-encrypt | 120,000 | 署名付きメッセージ |

上限はアルゴリズム別であり、JavaScript の文字数ではなく UTF-8 bytes
で数えます。ポスト量子経路は複数 OCF2 を使い、120,000 bytes まで受け付けます。
A256GCM は単一の v1 OCM1 ペイロードのままなので、暗号化前のより小さい上限を
v1 の 8,192 文字ペイロード上限と、選択した version 40 QR 誤り訂正容量から
導出します。
...
* 表示: WebAssembly reader が利用可能なら 1,000B chunk / 最低 200ms dwell、利用不能なら best-effort の 100B / 最低 2,000ms fallback をアプリが選ぶ。成果物が収まらない場合は密度を自動的に引き上げ、密度・速度の利用者設定は設けない
* 切替: 描画済みコードの commit 後にのみ cursor を進め、その後に設定 dwell を全て与える。dwell は下限であり、実測 frame rate や実 cycle time ではない
* 読取: 順不同・重複無視。欠損フレームは UI で明示。別 transfer 混入は `FRAME_MISMATCH`
* 出力: Download 操作は 1 frame なら PNG、複数 frame なら store-only ZIP を 1 つ生成する。SVG 出力は提供しない
...
* frame 密度・表示速度の設定、個別の出力ボタン、SVG 出力を廃止しました。表示 profile はアプリが選び、1 つの Download 操作が 1 frame では PNG、複数 frame では ZIP を生成します。
...
| QR のアプリ内保存機能なし | オーナー要件 2026-07-24。暗号文・鍵系 QR とも表示・単一 frame PNG／複数 frame ZIP エクスポート・クリップボードのみとし、保存済み QR 機能と `qrArtifacts` store は廃止 |
