...
利用者自身が出力した PNG / SVG / ZIP やクリップボードの内容はアプリの管理外であり、wipe の
対象外です。
...
鍵素材の取り扱いもしません。この外側ヘッダーは強制力のある保証ではありません。ラベルを付け替えた
成果物はフィルターを通過し、オフライン端末が組み立てた後にはじめて拒否されます。成果物を認証するのは
オフライン端末だけです（[docs/security/threat-model.md](docs/security/threat-model.md) T19）。
