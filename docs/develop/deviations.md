...
| `wrangler` kept as a devDependency though no script invokes it | `cloudflare/wrangler-action@v4` in `.github/workflows/cloudflare-pages.yml` uses the project's wrangler when one is present, so the devDependency is what pins the deploy tool's version (see [../security/security-review.md](../security/security-review.md) §1 for the 4.114.0 bump) |
...
| Online use limited to installation plus the clean-origin optical relay | Maintainer decision: online encryption/decryption/key-management/settings stay blocked. A fail-closed clean-origin relay may forward exact canonical OCF2 strings whose untrusted outer header declares `pq-message`, or one canonical OCM1 message after a complete decode and byte-for-byte canonical re-encode equality check ([../security/threat-model.md](../security/threat-model.md) T19). The OCM1 check establishes structural canonicality only; the relay performs no OCF2 assembly, completed-artifact total-hash check, AEAD or signature verification, or decryption. On the offline→online transition, in-memory transient data is cleared immediately |
| Replay detection is session-only | Maintainer decision, 2026-07-28. Persisting a frame- or OCM1-derived receipt (ciphertext hash / message ID) would violate the no-frame-or-OCM1-derived-persistence invariant ([../security/threat-model.md](../security/threat-model.md) §1 / T11 / T19) and the clean-origin boot gate. Cross-session detection remains an open security-design decision in [../security/security-review.md](../security/security-review.md) |
