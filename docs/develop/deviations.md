...
| Deviation | Reason / basis |
| --- | --- |
| Active vocabulary is the single PQ suite `ML-KEM-1024+ML-DSA-87+HKDF-SHA256+A256GCM` plus `SymSuite` `HKDF-SHA256+A256GCM` | Owner decision, 2026-08-01 (no-compatibility purge). Removed identifiers (unsigned suites, 768/65, `balanced`, v1 prefixes, RSA) are rejected at every boundary; there is no retained four-suite wire/codec contract |
