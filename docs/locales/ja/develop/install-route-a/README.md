...
これ未満の Cosign をこの手順に使用してはならない。脆弱な版では、差し替えられたレガシー形式のバンドルにより、以下の `--certificate-identity` と `--certificate-oidc-issuer` ポリシーが黙って迂回される可能性がある。したがって最低バージョンは独立に認証する検証ポリシーの一部であり、Cosign バイナリ自体を独立に入手・認証する要件は変わらない。
