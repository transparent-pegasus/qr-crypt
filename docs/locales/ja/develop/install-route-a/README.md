...
```
   ...
   いかなる差分も改竄の兆候である：中止してリリースを拒否すること。アーカイブのコピーをローカルで「修復」してはならない。その上で、再生成されたファイルをこのドキュメントと一度突き合わせて読む。バイト一致は「アーカイブが認証済みソースの記述どおりである」ことを既に証明しているので、この読み合わせが確認するのはアーカイブではなくソース自体である — すなわち §4 の展開前検証と、§7 の監査済み・事前インストール済み・独立に取得されたサーバーを、いまも要求しているか。
