...
高保証オフラインインストールの完全な手順。アーカイブ内の `INSTALL.txt` は、オフラインデバイスに届く自己完結型のコピーである。独立検証のためには、認証済みソースコミットにあるこのドキュメントのコピーが正本となる。アーカイブの手順をこのドキュメントと照合し、要件が追加・省略・弱体化・その他変更されている場合は改竄の兆候として扱うこと。`INSTALL.txt` はバージョン管理されたテンプレート [`INSTALL.template.txt`](../../../../develop/install-route-a/INSTALL.template.txt) から [`scripts/generate-install-txt.mjs`](../../../../../scripts/generate-install-txt.mjs) が描画する。したがって §5 は、他のペイロードファイルと同様に、認証済みチェックアウトから当該メンバーをバイト単位で再現する。
...
方法B は受信者が実行可能な完全性チェックを提供しない。オリジン、TLS、または CDN を制御する攻撃者は、特定のデバイスに対して改変されたバンドルを配信でき、Service Worker がそれを永続化する。その後オフラインにしても改変は元に戻らない。改竄されたビルドは RNG を弱体化させ、ロードされた公開鍵を差し替え、暗号文に見せかけたデータに平文を埋め込み、ユーザーに通常の `OCF2:` フレームとしてそれを持ち出させることができる。この隠密搬出シナリオは [docs/security/threat-model.md](../../../../security/threat-model.md) の **T21** である。**T19** はリレーの仕組み（`pq-message` \| `sym-message` の OCF2 許可リスト、再生前の組み立て成果物スキーマ検証、オンラインホップでの AEAD の不在、フレーム由来または成果物由来のアプリ永続化の不在）のみを対象としており、インストールの完全性は対象外である。**T21** は検証では閉じない残差、すなわち正規形式で送信者が制御できる暗号文・`transferId`・`iv`・`createdAt` の値に隠されたデータを明記する。

## 2. 独立に認証された入力 — 必須

ZIP、その署名バンドル、`SHA256SUMS`、リリースページ、転送メディア、およびこのドキュメントは、検証が成功するまで信頼できないものとして扱うこと。以下のすべてを、ダウンロードとは独立したチャネルを通じて独立に入手・認証すること：

- 認証済みの Cosign バイナリ（v3.1.3 以上）
...
`GHSA-fx35-mq7g-6g98` は Cosign v3 の v3.1.2 以下に影響するため、この手順の最低版は v3.1.3 である。影響を受ける版では、差し替えられたレガシー形式のバンドルにより、以下の `--certificate-identity` と `--certificate-oidc-issuer` ポリシーが黙って迂回される可能性がある。したがって最低バージョンは独立に認証する検証ポリシーの一部であり、Cosign バイナリ自体を独立に入手・認証する要件は変わらない。
...
3. アプリケーションペイロードの全ファイルセットとすべてのペイロードバイトを比較する。アーカイブの全メンバーを網羅すること。除外してよいのは下記 2 つと、オンライン専用のランディングページ `about/` だけである。`about/` はステージング前にリリースワークフローが `dist/` から削除している（外部リンクを含み、エアギャップ端末に置く意味がない）。リビルド側には `about/` が残るため、これをアーカイブと比較すると正規のリリースでも必ず差分が出る：
...
4. 認証済みチェックアウトから `INSTALL.txt` を再生成し、バイト単位で比較する。生成器はソースコミットを独立に認証した値から受け取り、リリースバージョンを `package.json` から、Cosign のバージョンをリリースワークフローから、いずれもそのチェックアウト内で読む。したがって検査対象のアーカイブ側の値が比較に還流することはない：
...
```
   ...
   いかなる差分も改竄の兆候である：中止してリリースを拒否すること。アーカイブのコピーをローカルで「修復」してはならない。その上で、再生成されたファイルをこのドキュメントと一度突き合わせて読む。バイト一致は「アーカイブが認証済みソースの記述どおりである」ことを既に証明しているので、この読み合わせが確認するのはアーカイブではなくソース自体である — すなわち §4 の展開前検証の契約、ステップ 5 のマニフェスト再構築、および §7 の監査済み・事前インストール済み・独立に取得されたサーバーを、いまも要求しているか。アーカイブ側のコピーはこれらの要件をそれ自体に記載しているため、`INSTALL.txt` しか持たない検証者も同じ手順を実行できる（`tests/unit/generate-install-txt.test.ts` がこれを固定する）。このドキュメントはより詳しい根拠を保持し、アーカイブ側はそれを簡略化してよいが、アーカイブに存在しない要件をこちらだけが持ってはならない。

5. アーカイブのマニフェストを信頼したり単にチェックするのではなく、`SHA256SUMS.files` をローカルで再生成する。独立にリビルドしたペイロードとステップ 4 で再生成した `INSTALL.txt` から期待ルートを組み立て、ソート済みマニフェストをローカルに生成し、マニフェストのバイトを比較し、最後に完全なルートを比較する。これによりペイロード、`INSTALL.txt`、および `SHA256SUMS.files` が網羅される：
...
リリースのブラウザゲートは再ビルドせず展開済みアーカイブを配信し、全スイートの
前後でSHA-256と全メンバーのバイト列を検査し、検査済みアーカイブのダイジェストを
署名・公開処理へ渡して比較する。別コピーのJavaScriptを壊すとブラウザ起動が失敗し、
正常なチェックアウトのビルドは起動する必要がある。これはチェックサム拒否とは別の検証。
これらはソース、ツールチェーン、ハードウェア、実際のサーバーを認証せず、上記の独立検証を
一つも省略させない。リリース担当のローカル証拠と未了の統合検証は
[deployment.md](../../../../develop/deployment.md)に記録する。

CI ゲートが証明**しない**こと：CI 内ダブルビルドは同一環境での決定論性のみを示すものであり、環境非依存の再現性を示すものではない。Cosign 署名は、当該ワークフローが当該アーティファクトを公開したこと（ワークフローおよびソースコミットの来歴）を証明する。いずれもソースからバイナリへの対応関係を確立するものではない。それを確立するのはこの手順だけであり、しかも実行した当事者に対してのみである：アーカイブの全メンバー — ペイロード、`INSTALL.txt`、マニフェスト — が認証済みソースコミットからリビルドまたは再生成され、ここでバイト比較される。
...
1. 検証済みアーカイブをオフラインデバイスに移動する。USB メモリや SD カードなど、何で運ぶにしても、それ自体も信頼できなければならない。ストレージを変更できるものは何でもアプリを変更できる。認証済みZIPや正常なファイルシステムは、媒体のコントローラやファームウェアを認証しない。信頼と管理の根拠を記録し、この脅威モデルを満たせない転送は拒否すること。
2. §4 で生成された、コンテナ検証済み・トラバーサル安全な展開結果のみを使用すること。ZIP は単一のディレクトリを作成する。そのディレクトリがドキュメントルートとなる。
3. `127.0.0.1` のみにバインドされた、**オフラインデバイスに事前インストール済みで、独立した信頼できるプロセスを通じて取得され、監査された静的サーバー**で配信する。「信頼できる」と呼ばれる経路でインストールしただけのサーバーは同等ではない。バンドルされた `_headers` および `_redirects` のセマンティクスを適用しなければならない：セキュリティヘッダ、正しい MIME タイプ、`/index.html` への SPA フォールバック、および到達性センチネルに対する `no-store`。Meta CSP フォールバックおよび `frame-ancestors` がヘッダ専用であること：[threat-model.md](../../../../security/threat-model.md) §2。衝突しやすいデフォルト（8000 や 8080 など）ではない、一般的でない固定の高ポートを1つ選び、そのポートを QR Crypt 用に予約すること。

   ほとんどの静的サーバーは `_headers` を全く解釈しない。アプリは、サーバーがまだ到達可能なうちに reachability センチネル応答のヘッダーを検査して判定を永続化し、判定が不合格または不在ならアプリケーションのマウントを**拒否する**。したがってヘッダーを落とすサーバーは、黙って劣化するのではなく fail-closed する。ただしこれは設定ミスの検出であって独立した保証ではない。検査対象はセンチネル応答だけなので、センチネルにはヘッダーを適用するが `/index.html` には適用しないサーバーは通過してしまう。期待ポリシーは同じチェックアウトから導出され、独立したセキュリティ下限ではない。手順4の実応答の独立検査を実施すること。サーバーが `_headers` を全く適用しない場合、CSP 以外の6つのセキュリティヘッダは単に存在しない。生き残るのは meta CSP フォールバックと `<meta name="referrer">` フォールバックだけである。失われるものの正確な一覧は [threat-model.md](../../../../security/threat-model.md) §2 を参照。ソースツリーの `scripts/serve-dist.mjs` が本リポジトリにおける必要な規則のリファレンス実装であり、Cloudflareのヘッダー言語全体を実装するものではない。ファイルの解析には `scripts/csp-from-headers.mjs` がエクスポートする共有パーサーを使うため、両方を読むこと。自分のサーバー設定を導出するために読むこと。検証対象の成果物を増やすだけなので、オフラインデバイスへ持ち込んではならない。これは署名済みリリースの一部ではなく Node のツールである。
4. インストールを受け入れる前に、独立に導入したチェッカーで、実際のサーバーのGET応答を検査する。対象は `/`、`/index.html`、SPAナビゲーション、スクリプト、スタイル、WASM、`/sw.js`、`/registerSW.js`。意図したセキュリティポリシー、MIMEとキャッシュ規則、センチネル本文と `no-store`、メソッド制限、パス封じ込めを確認する。アプリが保存するセンチネル判定は、これら他の応答やブラウザでの強制を検証できない。リポジトリの参照サーバーでのリリースブラウザテストは別の証拠であり、この検査の代わりにならない。その後、正確な `http://127.0.0.1:PORT` オリジンを開き、アプリがオフライン使用の準備完了を報告するまで待つ。
5. サーバーを停止し、転送メディアを取り外し、ネットワークを物理的に切断し、秘密情報の入力・復元の**前に** QR Crypt がオフラインと報告していることを確認する。インストールサーバー自体のセンチネルは意図的にアプリからそのオリジンを到達可能として扱わせるため、サーバー動作中に秘密情報を入力してはならない。
   ...
   順序が重要であり、アプリはそれを強制するようになった。サーバーを停止した後、ネットワークを切断する前に再読み込みすると、ブラウザが接続を報告している一方でアプリは自身のオリジンに到達できないため、**「ネットワーク接続を検出しました」** でロックし、開かない。これは不具合ではなく意図した指示である。VPN やコンテナブリッジのような仮想インターフェース（これらもブラウザに接続ありと報告させる）を含めてネットワークを切断し、再読み込みすること。何も削除されず、保存済みの鍵は無傷である。

   同じ順序はワイプ後にも適用される。`wipe-on-online` リセットが成功すると、アプリケーションデータベースとともに保存済みのデプロイ判定が取り除かれるため、インストールサーバーへ到達できないワイプ済みデバイスは、開く代わりに **「インストールが未検証です」** でロックする。これは不具合ではなく fail-closed 状態であり、それ以上何も削除しない。アプリが再検証できるよう、同じ `http://127.0.0.1:PORT` から再びオリジンを配信し、その後この手順どおりにサーバーを停止してネットワークを切断すること。
