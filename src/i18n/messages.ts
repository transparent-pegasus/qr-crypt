...
const en = {
  ...
  "relay.boundary.body":
    "This relay accepts only encrypted message frames (OCF2 pq-message or sym-message). It checks their format, but never decrypts, verifies, stores, or transmits them — anything they carry, including hidden data, stays untrusted until the receiving offline device authenticates it. Exchange keys face to face, never through this relay.",
...
const ja = {
  ...
  "relay.boundary.body":
    "このリレーは暗号化済みメッセージフレーム(OCF2のpq-messageまたはsym-message)だけを受け入れます。形式は検査しますが、復号・検証・保存・送信は行いません。運ばれる内容は、隠れたデータを含め、受信側のオフライン端末が認証するまで信頼できません。鍵の交換は必ず対面で行い、このリレーでは行わないでください。",
