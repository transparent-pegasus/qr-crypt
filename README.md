...
**しないこと**: 平文や秘密鍵の外部送信、クラウド上の鍵保管、独自暗号アルゴリズム、CDN 依存のランタイム、オフライン表示を安全性の証明として扱うこと、暗号文 QR の永続保存（オーナー要件。表示・エクスポートのみ）。
...
* **サーバーを介さないエアギャップ鍵交換** — 鍵・公開鍵・暗号文をすべて QR として扱い、クラウドの鍵預託やアカウント同期を用いません。鍵は端末内の IndexedDB のみに置き、平文で外部保存しません。暗号文は保存せず、その場の表示・PNG/SVG/ZIP エクスポートのみです。
